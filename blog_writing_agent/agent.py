...
If `codebase_context` exists in state, wave in specific sections/snippets.
...
blog_writer = Agent(
    ...
    instruction="""
Writes a complete Markdown article from the outline in `blog_outline`.
...
conclusion, abd technical clarity.
If passes, respnd "ok". Else respond "retry" with the specific fixes.
""" ,
            output_key="validation_resutl",
        )
